...
class Router {
  ...
  midlewear(req, res, next) {
    ...
    if (typeof this.options.router.header === 'undefined') {
      next();
      return;
    }

    if (typeof this.options.router.header.allowedMethods !== 'undefined') {
      res.setHeader('Access-Control-Allow-Methods', this.options.router.header.allowedMethods);
    } else {
      res.setHeader('Access-Control-Allow-Methods', 'POST, GET, PUT, DELETE');
    }

    if (typeof this.options.router.header.allowedHost !== 'undefined') {
      res.setHeader('Access-Control-Allow-Origin', this.options.router.header.allowedHost);
    } else {
      res.setHeader('Access-Control-Allow-Origin', '*');
    }

    if (typeof this.options.router.header.allowedHeaders !== 'undefined') {
      res.setHeader('Access-Control-Allow-Headers', this.options.router.header.allowedHeaders);
    }

    if (typeof this.options.router.header.allowedCredentials !== 'undefined') {
  ...
  createServer() {
    ...
    if (this.options.router.ssl) {
      this.logger.info('Enable HTTPS Mode');
      ...
      this.logger.info('Enable HTTP Mode');
