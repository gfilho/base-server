...
class Router {
  ...
  start() {
    ...
    if (this.options.router.ssl) {
      ...
      https.createServer(sslCredentials, this.app).listen(this.options.router.port);
    } else {
      this.logger.info('Start HTTP Mode');
      http.createServer(this.app).listen(this.options.router.port);
    }
  }
